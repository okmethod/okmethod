...
別のデッキに差し替える場合はこのファイルを置き換える。
main.py が期待するインターフェースは以下の5つ:
    OWN_DECK               : デッキリスト（60枚のカードID）
    collect_context(obs)           -> GameContext
    update_attack_plan(obs, ctx, state) -> None
    score_option(obs, o, ctx, state)    -> int
    post_pick(obs, top_option, state)   -> None
"""
...
def _energy_score(pokemon: Pokemon, active: bool, ctx: GameContext) -> int:
...
def collect_context(obs: Observation) -> GameContext:
    state = obs.current
...
def update_attack_plan(obs: Observation, ctx: GameContext, state: AgentState) -> None:
...
def _score_card(obs: Observation, o, ctx: GameContext, state: AgentState) -> int:
    ...
    if context in (SelectContext.SWITCH, SelectContext.TO_ACTIVE):
        ...
        if card.id == Makuhita:
            ...
            score += (
                30 if not state.ability_used or not game_state.energyAttached else -1
            )
    elif context == SelectContext.ATTACH_FROM:
        score = _energy_score(card, o.area == AreaType.ACTIVE, ctx)

    return score


def _score_play(obs: Observation, o, ctx: GameContext, state: AgentState) -> int:
    ...
    if data.cardType == CardType.POKEMON:
        ...
        if card.id == Riolu:
            return (
                -1
                if ctx.field_counts[card.id] + ctx.field_counts[Mega_Lucario_ex] >= 2
                else 20000
            )
        return 20000
...
def _score_attach(obs: Observation, o, ctx: GameContext, state: AgentState) -> int:
    card = get_card(obs, AreaType.HAND, o.index, ctx.own_index)
    ...
    score = _energy_score(pokemon, o.inPlayArea == AreaType.ACTIVE, ctx)
...
def _score_evolve(obs: Observation, o, ctx: GameContext, state: AgentState) -> int:
    pokemon = get_card(obs, o.inPlayArea, o.inPlayIndex, ctx.own_index)
...
def _score_ability(obs: Observation, o, ctx: GameContext) -> int:
    card = get_card(obs, o.area, o.index, ctx.own_index)
    return 1 if card.id == 1267 else 30000  # 1267: ルミオスシティ


def _score_attack(o, state: AgentState) -> int:
    score = 1000
...
def score_option(obs: Observation, o, ctx: GameContext, state: AgentState) -> int:
    ...
    match o.type:
        ...
        case OptionType.CARD:
            return _score_card(obs, o, ctx, state)
        case OptionType.PLAY:
            return _score_play(obs, o, ctx, state)
        case OptionType.ATTACH:
            return _score_attach(obs, o, ctx, state)
        case OptionType.EVOLVE:
            return _score_evolve(obs, o, ctx, state)
        case OptionType.ABILITY:
            return _score_ability(obs, o, ctx)
...
def post_pick(obs: Observation, top_option, state: AgentState) -> None:
