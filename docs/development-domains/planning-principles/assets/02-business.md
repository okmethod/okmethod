...
- ビジネス側（ドメインエキスパート）と開発側で、同じ言葉を同じ意味で使えるよう、用語集を作成する
  ...
  - ただし、ドメインエキスパートが存在しない新規事業領域等では、定義を急ぐと現実と乖離した用語集になりやすい点には注意 - 言語を「定義する」のではなくプロダクトと並走しながら「発見していく」姿勢が必要になる（ドメイン知識は本質的に暗黙知）
...
- ただし、短期 PoC・廃棄前提のプロトタイプでは、費用対効果を考慮した範囲に留める（最低限の言葉合わせで進め、本格開発移行時に整備する等）
