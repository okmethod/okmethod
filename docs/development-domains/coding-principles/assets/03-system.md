...
- 想定されるエラーの扱いを事前に取り決めしておき、エラーが発生した場合でも正常に終了させる（＝異常終了しないようにする）
  ...
  - ただし、例外処理は乱用せず、正常処理で対応できる範囲は正常処理として書く
    ...
    - 例: 在庫なし・重複登録など、仕様上エラーが返ることが想定内の場合は、例外処理ではなく正常系の制御フローとして扱う（何を「例外」と見なすかは業務コンテキストに依存する）
- 問題を早期に検出し、中途半端な状態のまま処理を続行したりせず、速やかに失敗させる（フェイルファスト）
  - 例: 不正な引数や null の必須パラメータを受け取ったら、処理を続けずに例外を投げる
  - ただし、バッチ処理で大量レコードを扱う場合は1件の失敗でバッチ全体をフェイルファストすると他の正常処理がすべてロールバックされる- エラーレコードをスキップして記録し後から再処理する「エラー許容型バッチ」が業務上適切なことがある
- エラー発生時は適切なログ記録とエラー通知を行う
...
- ログ記録・エラー通知など副作用のある対応は、最上位レイヤー（コントローラー/API エンドポイント）に集約する
  - 下位レイヤーは技術的な例外の変換やエラーメッセージ作成など、共通処理のうち副作用を伴わない部分を担当させ、役割分担するのが望ましい
...
### 相関 ID（Correlation ID）を全処理に付与する

- すべてのログに共通の「相関 ID（リクエスト ID）」を付与し、一連の処理の流れを串刺しで検索できるようにする
  - マイクロサービスや非同期処理では、1 つのリクエストが複数のサービス・ワーカーを横断するため、同一リクエストのログを追うには共通 ID が必須になる
  - リクエスト受付時に UUID を生成し、後続の全処理・外部呼び出しに引き継ぐ
  - 例: HTTP ヘッダーの `X-Correlation-ID` として付与し、サービス間で伝播させる
- ユーザー ID やセッション ID も合わせて付与すると、ユーザー単位での操作ログの追跡が容易になる
...
> Web アプリケーション固有の攻撃手法と対策（XSS・CSRF・SQL インジェクション等）の網羅的な解説 → [Web セキュリティ概要](../../../references/web-security-overview.md)
...
> 本セクションのスコープは、**実装時に意識すべきコーディング観点**（テストしやすい設計・テストコードの品質）に限定する。
> テスト戦略・テスト層設計・検証フローの詳細は [テスト設計の心得](../../testing-principles/README.md) を参照。
...
- テストコードは DRY より DAMP（Descriptive And Meaningful Phrases）を優先する
  - プロダクションコードは DRY（抽象化・共通化）を重視するが、テストコードは「読んだ瞬間に意図が伝わること」を優先する
  - 多少の繰り返しがあっても、ヘルパーへの過剰な抽象化より「テストケース内に必要な情報が揃っている」方が保守しやすい
  - 例: セットアップを共通ヘルパーに切り出しすぎると、テスト失敗時に何が起きているかを理解するために複数箇所を読み回ることになる
