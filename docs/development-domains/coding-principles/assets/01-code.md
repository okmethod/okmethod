...
- テストファイルや環境設定ファイルなど、一般的な慣習があるものはそれに従うこと
- ただし、コンテキストが自明な場合は記号・短い名前を許容する
  - 例: 数学・物理・統計系コードでの論文・数式由来の記号（`θ`/`theta`、`α`/`alpha` 等） - ドメイン知識共有が前提
  - 例: ソートアルゴリズムの `i`、`j` のようにスコープが数行に限定されるケース
...
- 型アノテーションを積極的に書く
  - 関数のシグネチャ（引数・戻り値）を中心に型を付ける
  - 型はコードに情報を付加する手段であり、命名と同様に意図を伝える
  - 例: `def get_user(user_id: int) -> User:`
- 曖昧な型を避け、型の精度を高める
  - `any` / `Any` など曖昧な型は最小限にする
  - Nullable は `str | None` のように明示する
- 型チェッカーを利用し、CI/CD に組み込む
  - Python: mypy, pyright
  - TypeScript: TS 自体が型チェッカーを内蔵（tsc）
