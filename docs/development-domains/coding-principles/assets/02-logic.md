...
**_制御フローを単純にし、読み手が立ち止まったり読み返したりしなくて済むようにする。_**
...
- ガード節で異常系を先に処理する
  ...
  - ただし、リソースの確保・解放が必要な場合などは、早期リターンでなく finally ブロックを使うべき場合もある点に注意
- 条件を反転して、ネストを削減する
...
**_関数やクラスは、簡潔かつ明確で見通しのよい設計にする。_**

### 一度に 1 つのことを

- 一度に 1 つのことだけを行う関数を心がける
  - 例: データの読み込み、変換、表示を 1 つの関数で行わず、それぞれ独立させる
- 単一責任の目安としては、関数名をシンプルに決められるかどうか？で判断するとよい
  - 良い例: 関数名に含まれている動詞（行為）が 1 つに限定されている
  - 悪い例: 関数名が長い or 動詞を 2 つ以上含んでいる（この場合、複数の責任を持っている可能性を疑うとよい）
...
- 副作用のない関数を実装する
  ...
  - コマンド（状態を変更する）とクエリ（値を返す）を分離する（CQS 原則）
- 同じ入力には常に同じ出力を返す（参照透過性）
  - 例: `calculate_price(items)` は同じ items に対して常に同じ価格を返す
  - 参照透過性により、関数のテストや並列処理が容易になる
