# アーキテクチャ設計の心得

## 大原則： 複雑性の主戦場を見極め、ベターな特殊解を見つけ出す！

- 正解のアーキテクチャ（銀の弾丸）は存在せず、その時点で最も良い（悪くない）ものを選ぶしかない - だからこそ、状況の変化に合わせて変化（進化）させやすいことが「良い設計」となる
- すべての具体的なアーキテクチャは個別解・特殊解であり、それを考えるために一般解（アーキテクチャパターン）を知る - ただしパターンありきではなく、まず「どこに複雑性が宿っているか（複雑性の主戦場）」を問い、それを解決する手段としてパターンを使う
- 考慮すべき品質特性（-ility）を知り、トレードオフを踏まえて意思決定するとともに、「なぜこの設計にしたのか」という背景と根拠を記録して将来の変更に備える

---

## 前提： 本ガイドにおける用語の定義について

システム開発において「設計（Design）」や「アーキテクチャ（Architecture）」という言葉は、文脈によって指す範囲が大きく異なる。  
本ガイドでは、「設計の粒度」と「変更に伴うコスト」に基づいて、以下の 3 つの階層に分類してまとめている。

- `Organization / 組織構造 / 物理境界`
  - 視点: 複数コンポーネントが連動・協調して動くシステム全体。組織構造やインフラ戦略と密接に関わる。
  - 役割: システムの物理的な境界線を引き、インフラや技術スタックの選定を通じてシステムの全体像を決定する。
  - 決定後の変更コスト: 特大
  - 強力な武器: アーキテクチャスタイル
  - キーワード: モノリス vs マイクロサービス、オンプレミス vs クラウドネイティブ
  - 参考: [アーキテクチャスタイル一覧](./assets/architecture-styles-cheatsheet.md)
- `Layout / 内部構造 / 論理境界`
  - 視点: 単一コンポーネントの内部構造。変更容易性やテスト容易性と密接に関わる。
  - 役割: 業務要件（ドメインモデル）をコード上で表現するための、論理的な分割（区画整理）と依存関係を整理する。
  - 決定後の変更コスト: 大
  - 強力な武器: アプリケーション・アーキテクチャパターン、構造に関する GoF デザインパターン
  - キーワード: レイヤードアーキテクチャ、ディレクトリ構成、依存の方向性
  - 参考: [GoF デザインパターン一覧](./assets/GoF-design-patterns-cheatsheet.md)
  - ケーススタディ: [Clean Architecture の実践と対比](./assets/clean-architecture-case-study.md)
- `Mechanics / 力学 / 横断的法則`
  - 視点: システムの「品質特性(-ility)」を支える、普遍的な物理法則または経験則。トレードオフの判断と密接に関わる。
  - 役割: システム全体の「横断的関心事」たる非機能要件（信頼性、整合性など）の優先順位とバランスを適切に決めるための戦略・判断基準を提供する。
  - 決定後の変更コスト: 中〜特大（層によって異なる）
  - 強力な武器: 統合パターン（EIP）・回復性パターン・可観測性パターン、振る舞いに関する GoF デザインパターン
  - キーワード: 横断的関心事（Cross-Cutting Concerns）、エラー時のリトライ方針、整合性モデル
  - 参考: [GoF デザインパターン一覧](./assets/GoF-design-patterns-cheatsheet.md)

また、前工程（要件定義）から後工程（実装）への流れの中で、本ガイドの位置付けは以下の通りとなる。

- Input: システム要件 (What) ・・・ [要件定義の心得](../planning-principles/README.md) 参照
- Process: アーキテクチャ設計 (How - 概念:Concept) ・・・本ガイド
- Output: コード実装 (How - 実体:Substance) ・・・ [コーディングの心得](../coding-principles/README.md)

---

## [organization: どのように組織構造の境界を設計するか](./assets/01-organization.md)

コンセプト: **_適切なアーキテクチャスタイルと技術スタックの選定ポイントを押さえ、システムの物理的な境界線を慎重に決定する_**

トピック:

- アーキテクチャスタイル選定: 組織の規模とフェーズに合わせて、システムの分割単位（デプロイメント単位）を決める
- 技術選定: 「新しさ」ではなく「信頼性」と「習熟度」で技術を選ぶ
- データストア選定: 業務ドメインのデータ特性を踏まえ、その手段として最適なデータストアを選ぶ
- サービス間通信プロトコル選定: 同期か非同期かを意識的に選び、サービス間の結合度をコントロールする
- 認証基盤選定: 差別化につながらない複雑性として、標準プロトコルと認証プロバイダに委譲する
- サーバー・クライアント責務分担: クライアントサイドは悪用・改竄される前提で設計し、重要責務はサーバーサイドに集約する
- インフラ構築: 手作業を排除し、再現可能なインフラを作ることをデプロイ戦略に組み込む

## [layout: どのように内部構造を区画整理するか](./assets/02-layout.md)

コンセプト: **_コンポーネント内部の関心を分離し、ドメインの複雑性をシンプルに表現する_**

トピック:

- レイヤー設計: 責務を分離し、依存の方向を制御して、変更容易性とテスト容易性を高める
- モジュール分割: 機能（ビジネスドメイン）単位でコードをまとめ、高凝集・低結合な構造を作る
- データモデル配置: ドメインの知識をどこに置くかを明確にする

## [mechanics: どのように品質特性のバランスをとるか](./assets/03-mechanics.md)

コンセプト: **_トレードオフの力学を理解し、システムに求められる品質特性(-ility)を実現する_**

トピック:

- 品質特性のトレードオフ: 制約が最も厳しい品質特性を主役に据え、周辺の設計を決める
- パフォーマンスとスケーラビリティ: 計測なしに最適化せず、スケールの壁を構造で突破する
- 整合性と信頼性: 分散システムにおける「完全な整合性」を諦め、現実的な解を選ぶ
- 可観測性: 「何が起きているか」ではなく「なぜ起きているか」を知るための仕組みを入れる
- セキュリティ: 「安全である」と過信せず、「侵害される前提」で多層防御する
- 回復性: 障害を前提とし、部分的な失敗がシステム全体に波及しないようにする

---

## 参考

- [O'Reilly Japan ソフトウェアアーキテクチャの基礎](https://www.oreilly.co.jp//books/9784873119823/)
- [Refactoring.Guru — Design Patterns](https://refactoring.guru/design-patterns)
- [ソフトウェアアーキテクトが知るべき 97 のこと](https://ソフトウェアアーキテクトが知るべき97のこと.com)
